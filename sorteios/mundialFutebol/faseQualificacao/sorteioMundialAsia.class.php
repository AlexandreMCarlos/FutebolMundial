...
Class sorteioMundialAsia{
		
	public $prefixoConf = 'MAC';
	protected $idConf = 1;
	protected $ofcConf = 5;

	public $prefixoFase = array('MAC_01', 'MAC_02', 'MAC_03');
	...
	* Acção: Vai dar inicio ao sorteio da fase de qualificação para o Mundial de Futebol (Zona Asiática) 
	...
	*/
	...
	public function __construct($confOrganizador, $mundialAno){
		...
		If ($this->mundialOrganizador['conf'] == $this->ofcConf){
			$varBD->queryString .= 'tab_equipas_conf in (:CONF, :OFC) and tab_equipas_trig <> :EQU';			
		}else
		...
		If ($this->mundialOrganizador['conf'] == $this->ofcConf){
			oci_bind_by_name($varBD->queryParse, ':OFC', $this->ofcConf);			
		}
