...
Class sorteioMundialAmSul{
		
	public $prefixoConf = 'MAM';
	protected $idConf = 4;

	public $prefixoFase = array('MAM_04');
	...
	* Acção: Vai dar inicio ao sorteio da fase de qualificação para o Mundial de Futebol (Zona Sul Americana) 
	...
	* Data: 09/03/2012
	* Autor: Alexandre M. Carlos
	* Ac��o: Fun��o que vai fazer o sorteio dos grupos para a fase de qualifica��o do Mundial (AFC)	
	...
	*/
	...
	Function funSorteiaGrupos($varPotes, $faseSorteio){
		...
		For($i = 0; $i < sizeof($varPotes); $i++){
		        $this->gruposSorteio[$faseSorteio-1][0]->funSetEquipaGrupo($varPotes[$i]);
		}
	}
	
	/*Function funSorteiaGrupos($varEquipas, $faseSorteio){
        	
		If($this->mundialOrganizador['conf'] == $this->idConf)	
			$controlo = 3;
		Else
			//$controlo = 1;
			$controlo = 3;	

        $varPotes = array_chunk($varEquipas, $controlo);

		for ($i = 0; $i < sizeof($varPotes); $i++){
			for ($j= 1; $j < $controlo; $j++){
				shuffle($varPotes[$i]);
			}
		}
